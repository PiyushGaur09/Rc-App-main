...
//     formDataToSend.append('is_customer', formData.areYouCustomer === '1' ? 1 : 0);
...
const DeliveryChallan = ({navigation, route}) => {
    ...
    const getUserData = async () => {
      try {
        ...
        if (route.params?.formData) {
          ...
          // Set payment entries if available
          if (editData.payment_entries) {
            setPaymentEntries(editData.payment_entries);
          }
  ...
  const prepareFormData = () => {
    ...
    paymentEntries.forEach((entry, index) => {
      if (entry.date && entry.by && entry.amount) {
        formDataToSend.append(`entry_date[${index}]`, entry.date);
        formDataToSend.append(`entry_by[${index}]`, entry.by);
        ...
        <View style={styles.inputContainer}>
          <LinearGradient
            ...
            <TextInput
              style={styles.textInput}
              value={entry.date}
              onChangeText={text => updatePaymentEntry(index, 'date', text)}
        ...
        <View style={styles.inputContainer}>
          <LinearGradient
            ...
            <TextInput
              style={styles.textInput}
              value={entry.by}
              onChangeText={text => updatePaymentEntry(index, 'by', text)}
        ...
        <View style={styles.inputContainer}>
          <LinearGradient
            ...
            <TextInput
              style={styles.textInput}
              value={entry.amount}
              onChangeText={text => updatePaymentEntry(index, 'amount', text)}
              placeholder="Amount"
              placeholderTextColor="#666"
              keyboardType="numeric"
              editable={!loading}
        ...
        <View style={styles.inputContainer}>
          <LinearGradient
            ...
            <TextInput
              style={styles.textInput}
              value={entry.remarks}
              onChangeText={text => updatePaymentEntry(index, 'remarks', text)}
              placeholder="Remarks"
              placeholderTextColor="#666"
              editable={!loading}
            />
      ...
      <ScrollView style={styles.scrollView}>
        ...
        <View style={styles.paymentEntriesSection}>
          <View style={styles.paymentEntriesHeader}>
            <Text style={styles.sectionHeading}>Payment Entries</Text>
            <TouchableOpacity
          ...
          {paymentEntries.map((entry, index) =>
            renderPaymentEntry(entry, index),
          )}
        </View>
