...
const Pdiformscreen = ({ navigation }) => {
  ...
  const fetchPdiData = async (userId, isRefresh = false) => {
    try {
      if (isRefresh) {
        setRefreshing(true);
      } else {
      ...
      if (response.data.status && response.data.pdi_deliveries) {
        setPdiData(response.data.pdi_deliveries);
        setFilteredData(response.data.pdi_deliveries);
      } else {
        Alert.alert('Info', 'No PDI forms found.');
      }
    } catch (error) {
  ...
  const handleSearch = (query) => {
    ...
    if (query.trim() === '') {
      setFilteredData(pdiData);
    } else {
      const filtered = pdiData.filter(item =>
  ...
  const handleFormPress = (formId, status) => {
    navigation.navigate('Pdiinternalpage', { 
      formId,
      status
    });
  };
      ...
      <View style={[styles.loadingContainer, { paddingTop: insets.top }]}>
        <LinearGradient
          ...
          <View style={styles.headerContent}>
            ...
            <Text style={styles.screenTitle}>PDI Forms</Text>
            <View style={styles.placeholder} />
          </View>
        </LinearGradient>
        <View style={styles.loader}>
          <ActivityIndicator size="large" color="#7E5EA9" />
          <Text style={styles.loadingText}>Loading PDI Forms...</Text>
        </View>
      </View>
    );
  }
      ...
      >
        ...
        <View style={styles.resultsContainer}>
          <Text style={styles.resultsText}>
            {filteredData.length} PDI Form{filteredData.length !== 1 ? 's' : ''} found
          </Text>
        ...
        {filteredData.length === 0 ? (
          <View style={styles.emptyContainer}>
            <Icon name="car-repair" size={60} color="#ccc" />
            <Text style={styles.emptyText}>
              {searchQuery ? 'No matching PDI forms found' : 'No PDI forms available'}
            </Text>
            {searchQuery ? (
          ...
          filteredData.map((item) => (
            ...
            >
              ...
              <View style={styles.formDetails}>
                ...
                <View style={styles.detailRow}>
                  <Icon name="engineering" size={16} color="#666" />
                  <Text style={styles.detailLabel}>Inspector: </Text>
                  <Text style={styles.detailValue} numberOfLines={1}>
                    {item.inspector_name || 'N/A'}
                  </Text>
                </View>
              </View>
        ...
        {filteredData.length > 0 && (
          <View style={styles.listFooter}>
            <Text style={styles.listFooterText}>
              {filteredData.length === pdiData.length 
                ? 'All forms loaded' 
                : `${filteredData.length} of ${pdiData.length} forms shown`}
            </Text>
          </View>
        )}
      </ScrollView>
