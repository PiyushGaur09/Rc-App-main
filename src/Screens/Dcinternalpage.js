...
const Dcinternalpage = ({navigation, route}) => {
  ...
  const fetchTractorModels = async () => {
    try {
      ...
      const response = await axios.get(TRACTOR_MODELS_API);
      
      if (response.data.status && response.data.data) {
        // Assuming the API returns an array of model names or objects
        if (Array.isArray(response.data.data)) {
          const models = response.data.data.map(item => 
            item.name || item.model || item.tractor_model || item
          );
  ...
  useEffect(() => {
    ...
    getUserData();
    fetchTractorModels(); // Fetch tractor models on component mount
            ...
            {loadingTractorModels ? (
              ...
              <LinearGradient
                ...
                {renderDropdownField(
                  ...
                  tractorModels.length > 0 ? 'Select Model' : 'No models available',
                )}
              </LinearGradient>
            )}
          </View>
...
const styles = StyleSheet.create({
  ...
  // Loading indicator for tractor models
  loadingIndicator: {
    alignItems: 'center',
    justifyContent: 'center',
    padding: 20,
